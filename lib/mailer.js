...
// With a noreply From address, replies still land somewhere a human reads.
...
async function send({ to, subject, html, text, attachments }) {
  ...
  const info = await transporter.sendMail({ from: FROM, replyTo: REPLY_TO || undefined, to: list.join(', '), subject, html, text, attachments });
