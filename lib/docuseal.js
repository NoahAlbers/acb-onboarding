...
// The ACB countersigner's slug/status on a submission (null when no ACB role exists).
async function getCollectorStatus(submissionId) {
  const sub = await ds(`/submissions/${submissionId}`);
  const acb = (sub.submitters || []).find((s) => s.role === 'ACB');
  if (!acb) return null;
  const docs = sub.documents || acb.documents || [];
  return { slug: acb.slug, completed_at: acb.completed_at || null, name: acb.name || null, document_url: docs.length ? docs[0].url : null };
}
...
module.exports = { enabled, signUrl, createEntitySubmission, getSubmissionStatus, getCollectorStatus, archiveSubmission, checkConnection };
