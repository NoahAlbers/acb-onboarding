...
async function buildAgreementPdf(session, entity) {
  ...
  value(session.contact_name, MARGIN + 70, 180);
  label('Email', 330);
  value(session.contact_email, 358, 200);
  y -= 20;

  label('Phone', MARGIN);
  value(session.contact_phone, MARGIN + 40, 150);
  label('Fax', 330);
...
async function buildAllAgreementsPdf(session, entities) {
  ...
  for (const entity of entities) {
    const bytes = await buildAgreementPdf(session, entity);
