...
async function buildAgreementPdf(session, entity) {
  ...
  // ---- Collector countersignature (owner signs on the review page) ----
  if (entity.collector_signature) {
    try {
      const base64 = entity.collector_signature.split(',').pop();
      const png = await doc.embedPng(Buffer.from(base64, 'base64'));
      const scale = Math.min(112 / png.width, 36 / png.height);
      page.drawImage(png, { x: 46, y: 57, width: png.width * scale, height: png.height * scale });
    } catch (e) { /* leave the line blank */ }
  }
  if (entity.collector_name && (entity.collector_signature || entity.collector_signed_at)) {
    put(`${entity.collector_name}${entity.collector_title ? ', ' + entity.collector_title : ''}`, 162, 58, 90, 7);
  }
  if (entity.collector_signed_at) put(fmtDate(entity.collector_signed_at), 330, 58, 105, 10);
